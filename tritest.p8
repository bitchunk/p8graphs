...
trifuncs = {
--{'pelogen_tri_cspr',"shiftalow(cspr)",flattable}
{'pelogen_tri_hvb2',"shiftalow(hvb)",flattable}
...
--,{'pelogen_tri_lowl',"shiftalow(lowl)",flattable}
...
--  compress = 
  ...
  local cx=64
--  local cy=64-time()*100)1
  local cy=64
--dbg(tostr(cy,1),tostr(1-(cy&0x0.ffff),1))
--		?64+r*cos(angle),0,8
--		?mid(r*sin(angle+.33),-t,t),0,120

  local f = {
--  	64+r*cos(angle)
  	cx+mid(r*cos(angle),-u-1,u+1)
  	,cy+mid(r*sin(angle),-t-1,t+1)
--  	,64+r*cos(angle+.33)
  	,cx+mid(r*cos(angle+.33),-u-1,u+1)
  	,cy+mid(r*sin(angle+.33),-t-1,t+1)
--  	,64+r*cos(angle+.67)
  	,cx+mid(r*cos(angle+.67),-u-1,u+1)
  	,cy+mid(r*sin(angle+.67),-t-1,t+1)
  	,col
  }
-- compress_y test
  fillp(0xaaaa.8)
...
function pelogen_tri_hvb2(l,t,c,m,r,b,col)
	color(col)
	local a=rectfill
	::_w_::
...
--
function pelogen_tri_cspr(l,t,c,m,r,b,col)
pelogen_tri_tclip(l,t,c,m,r,b,14)
	...
	local hvr=c+hv*j
dbg(l,sl,hvl,hvr)
		for t=t\1,(m-hv)\1-1 do
			rectfill(l-min(min(sl,hvl),999),t-st,e-min(min(sl,hvl),hvr),t-st)
			l+=i
			e+=j
		end
		pset(0,0,7)
	poke(0x5f54,0x00,0x60)
		...
		spr(0,0,0,16,16)
--		spr(0,i*hv,t+hv,16,16)
--		spr(0,j*hv,t+hv,16,16)
--		spr(0,l+e-128,(t+hv*2)-130,16,16,true,true)
end

--
function pelogen_tri_csp(l,t,c,m,r,b,col)
--	pelogen_tri_tclip(l,t,c,m,r,b,8)
	color(col)
	::_w_::
	if(t>m)l,t,c,m=c,m,l,t
	if m>b then
